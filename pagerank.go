...
//
// For a detailed walkthrough of the original example, see
//
...
//
// ========================================================
// DC: This modified program will calculate page rank.
package main
...
var (
	...
	// DC: Start with something easy just to get started.............
	// DC: input = flag.String("input", "web04/README.md", "File(s) to read.")
	input = flag.String("input", "files.txt", "File(s) to read.")

	// Set this required option to specify where to write the output.
	//output = flag.String("output", "", "Output file (required).")
	// DC: Set the output file name for convenience..........
	output = flag.String("output", "out.csv", "Output file (required).")
)
...
// DC: Try the insides on one file first...
// DC: Then move to a reusable function that we'll call on each input file
func mapInputFile(s beam.Scope, inputFolder string, inputFile string) beam.PCollection {

	// combine the folder and the file name so we can read and process the web page
	pagePath := inputFolder + inputFile

	// Read in all the lines of the web page Markdown file
	linesPCol := textio.Read(s, pagePath)

	// Filter to just the lines that describe links (simple version)
	linklinesPCol := filter.Include(s, linesPCol, func(s string) bool {
		return strings.HasPrefix(s, "[")
	})

	// Markdown link lines will look like this: [README](README.md)
	// Use ParDo (like "map") to get just the outlink page name, README.md.
	linksPCol := beam.ParDo(s, func(fullLinkString string) string {
		// return just the part inside the parentheses
		cleanString := strings.TrimSpace(fullLinkString)
		openingChar := "("
		closingChar := ")"
		iStart := strings.Index(cleanString, openingChar)
		iClose := strings.Index(cleanString, closingChar)
...
func main() {
	...
	p := beam.NewPipeline()
	s := p.Root() // s refers to our beam.Scope

	// To read data from disparate sources into a single PCollection,
	// read each one independently and then use the Flatten transform
	// to create a single PCollection.
	//https://beam.apache.org/documentation/programming-guide/#file-based-reading-multiple-locations
